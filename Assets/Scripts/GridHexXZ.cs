...
{
    private const float HEX_VERTICAL_OFFSET_MULTIPLIER = 0.87f;
    ...
    public GridHexXZ(int width, int height, float cellSize, Vector3 originPosition, Func<GridHexXZ<TGridObject>, int, int, TGridObject> createGridObject)
    {
        ...
        //         for (int z = 0; z < gridArray.GetLength(1); z++)
        //         {
        //             // var textMesh = new GameObject("Text").AddComponent<TextMesh>();
        //             // textMesh.text = gridArray[x, z]?.ToString();
        //             // textMesh.transform.position = GetWorldPosition(x, z) + new Vector3(cellSize, 0, cellSize) * .5f;
        //             // textMesh.anchor = TextAnchor.MiddleCenter;
        //             // textMesh.alignment = TextAlignment.Center;
        //
        //             // debugTextArray[x, z] = textMesh; //UtilsClass.CreateWorldText(gridArray[x, z]?.ToString(), null, GetWorldPosition(x, z) + new Vector3(cellSize, 0, cellSize) * .5f, 40, Color.white, TextAnchor.MiddleCenter, TextAlignment.Center);
